...
export class WyvernExchange extends ethereum.SmartContract {
  ...
  calculateMatchPrice_(
    ...
    let result = super.call(
      ...
      [
        ethereum.Value.fromFixedSizedArray(addressArrayToAddressValueArray(addrs)),
        ethereum.Value.fromFixedSizedArray(bigIntArrayToBigIntValueArray(uints)),
        ethereum.Value.fromFixedSizedArray(i32ArrayToBigIntValueArray(feeMethodsSidesKindsHowToCalls)),
        ethereum.Value.fromBytes(calldataBuy),
        ethereum.Value.fromBytes(calldataSell),
        ethereum.Value.fromBytes(replacementPatternBuy),
        ethereum.Value.fromBytes(replacementPatternSell),
...
// MANUALLY WRITTEN CODE TO ADDRESS POTENTIAL CODEGEN ISSUE WITH GRAPH-TS
// see github issue: https://github.com/graphprotocol/graph-ts/issues/237 for more
const addressArrayToAddressValueArray = (values: Array<Address>): ethereum.Value[] => {
  let out = new Array<ethereum.Value>(values.length)
  for (let i: i32 = 0; i < values.length; i++) {
    out[i] = ethereum.Value.fromAddress(values[i])
  }
  return out
}
const bigIntArrayToBigIntValueArray = (values: Array<BigInt>): ethereum.Value[] => {
  let out = new Array<ethereum.Value>(values.length)
  for (let i: i32 = 0; i < values.length; i++) {
    out[i] = ethereum.Value.fromUnsignedBigInt(values[i])
  }
  return out
}
const i32ArrayToBigIntValueArray = (values: Array<i32>): ethereum.Value[] => {
  let out = new Array<ethereum.Value>(values.length)
  for (let i: i32 = 0; i < values.length; i++) {
    out[i] = ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(values[i]))
  }
  return out
}